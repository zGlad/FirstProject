...
def iniciar_aplicacion()->None:
    continuar=True
    datos = None # Inicializamos en vacio previniendo que se consulte un menu sin cargar csv
    while continuar:
        menu()
        elegido=int(input("Seleccione una opcion del menu: "))
        if elegido==0:
            datos=ejecutar_cargar_datos()
        elif elegido==1 and datos is not None:
            ejecutar_histograma_descubrimientos(datos)
        elif elegido==2 and datos is not None:
            ejecutar_estado_publicacion_por_descubrimiento(datos)
        elif elegido==3 and datos is not None:
            ejecutar_deteccion_por_descubrimiento(datos)
        elif elegido==4 and datos is not None:
            ejecutar_deteccion_y_descubrimiento(datos)
        elif elegido==5 and datos is not None:
            ejecutar_cantidad_y_tipo_deteccion(datos)
        elif elegido==6 and datos is not None:
            ejecutar_masa_promedio_y_tipo_deteccion(datos)
        elif elegido==7 and datos is not None:
           ejecutar_masa_planetas_vs_masa_estrellas(datos)
        elif elegido==8 and datos is not None:
            ejecutar_graficar_cielo(datos)
        elif elegido==9 and datos is not None:
            ejecutar_filtrar_imagen_cielo(datos)
        elif elegido==10:
            continuar=False
        elif datos is None and elegido != 0:
            print("Por favor, asegúrese de cargar los datos primero (Opción 0).")
        else:
            print("Seleccione una opcion del menu valida.")
